...
df.to_csv(f'gs://{DATA_BUCKET}/data/data_no_tweet.tsv', sep='\t')

df_curr, df_future = train_test_split(df, test_size=0.2)
df_future.to_csv(f'gs://{DATA_BUCKET}/data/data_future.tsv', sep='\t')

df_train, df_test = train_test_split(
    df_curr, test_size=0.2, stratify=df_curr.sentiment
)

df_train.to_csv(f'gs://{DATA_BUCKET}/data/data_train.tsv', sep='\t')
df_test.to_csv(f'gs://{DATA_BUCKET}/data/data_test.tsv', sep='\t')
